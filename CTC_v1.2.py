...
def cruzar_archivos():
    ...
    consolidado_pbs = pd.read_csv(filename, delimiter=';', encoding='latin1', low_memory=False)
    ...
    archivos_excel_ctc = glob.glob(os.path.join(ruta_base, patron))
    ...
    for ruta_archivo in archivos_excel_ctc:
        if ruta_archivo.endswith('.xlsx'):
            ...
            df_ctc.loc[filtro_fecha_envio & ~filtro_envio_sura, 'FECHA DE ENVIÓ ARUS'] = pd.to_datetime(
                df_ctc['G']).dt.strftime('%d/%m/%Y')
            df_ctc.loc[filtro_fecha_envio & filtro_envio_sura, 'FECHA DE ENVIÓ ARUS'] = pd.to_datetime(
                df_ctc['G']).dt.strftime('%d/%m/%Y')
    ...
    archivo_consolidado_final = r'C:\Users\santiago.patino\OneDrive - arus.com.co\ASIGNACION PBS\CONSOLIDADO_PBS\CONSOLIDADO_GENERAL_PBS_FINAL.xlsx'
...
def generar_carpeta_asignacion():
    ...
    carpeta_diario = r"C:\Users\santiago.patino\OneDrive - arus.com.co\ASIGNACION PBS\DIARIO"
